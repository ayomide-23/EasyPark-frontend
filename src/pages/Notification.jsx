...
const Notifications = () => {
  ...
  const fetchNotifications = () => {
    ...
    if (!token) return;
    fetch('http://localhost/PMS/fetch_notifications.php', {
  ...
  const handleMarkAsRead = async (id) => {
    try {
      await fetch('http://localhost/PMS/mark_notifications.php', {
        method: 'POST',
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await fetch('http://localhost/PMS/delete_notification.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ id }),
      });
